...
[Showcase website / 网页版介绍](https://doro1106.github.io/ComfyUI-Zcut/)

ComfyUI-Zcut is a ComfyUI custom node plugin for portrait cutout, face-centered avatar cropping, circular/square alpha masks, batch image loading, image upscaling, final output resizing, and background compositing.
...
The repository also excludes local model files, workflow files, standalone HTML drafts, and Codex maintenance scratch files: `models/`, `workflows/`, `HTML/`, and `codex_maintenance_guide/`.
...
ComfyUI-Zcut 是一个用于 ComfyUI 的自定义节点插件，用于人物抠图、头像居中裁切、圆形/方形透明蒙版裁切、批量图片加载、图像放大、最终输出尺寸调整和背景合成。
...
仓库也不会上传本地模型文件、工作流文件、独立 HTML 草稿和 Codex 维护临时文件：`models/`、`workflows/`、`HTML/` 和 `codex_maintenance_guide/`。
